...
void Renderer::RenderMeshWithTextures(Ref<Shader> shader, Ref<Mesh> mesh, const std::vector<Ref<SceneTextureData>>& textures, const glm::mat4& transform, float alphaTransparency, bool debugMode)
{
	...
	if (debugMode)
	{
		...
		mesh->GetBoundingBox().Draw(shader, transform);
	}
	else
	{
	...
	for (it = mesh->GetSubmeshes().begin(); it != mesh->GetSubmeshes().end(); it++)
	{
		...
		shader->SetMat4x4("matModel", submeshTransform);
		shader->SetMat4x4("matModelInverseTranspose", glm::inverse(submeshTransform));

		submesh.vertexArray->Bind();
		glDrawElements(GL_TRIANGLES, submesh.indexBuffer->GetCount(), GL_UNSIGNED_INT, 0); // TODO: Make an instanced rendering version of this
...
void Renderer::RenderSubmeshWithColorOverride(Ref<Shader> shader, const Submesh& submesh, const glm::mat4& transform, const glm::vec3& colorOverride, bool debugMode, bool ignoreLight)
{
	shader->Bind();
	...
	{
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
		submesh.boundingBox.Draw(shader, transform);
	}
	else
	{
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void Renderer::RenderSubmesh(Ref<Shader> shader, const Submesh& submesh, const glm::mat4& transform, Ref<SubmeshData> submeshData, bool debugMode)
{
	shader->Bind();
	shader->SetMat4x4("matModel", transform);
	shader->SetMat4x4("matModelInverseTranspose", glm::inverse(transform));
	...
	// Bind textures
	const std::vector<Ref<SceneTextureData>>& textures = submeshData->textures;
	int diffuseTextureindex = 0;
	for (int i = 0; i < textures.size(); i++)
	{
		const Ref<SceneTextureData>& textureData = textures[i];
	...
	glUniform1f(alphaTransparencyUniform, submeshData->alphaTransparency);

	if (debugMode)
	{
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
		submeshData->boundingBox.Draw(shader, transform);
	}
	else
	{
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
