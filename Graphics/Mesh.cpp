...
Mesh::Mesh(const std::string& filePath)
	: filePath(filePath), boundingBox(glm::vec3(FLT_MAX, FLT_MAX, FLT_MAX), glm::vec3(-FLT_MAX, -FLT_MAX, -FLT_MAX))
{
	...
	this->transform = ConvertToGLMMat4(scene->mRootNode->mTransformation);
	...
	std::vector<std::string> tempSubMeshVector(scene->mNumMeshes);
	for (unsigned int i = 0; i < scene->mNumMeshes; i++)
	{
		aiMesh* assimpMesh = scene->mMeshes[i];
		Submesh submesh = LoadSubmesh(assimpMesh);
		this->submeshes.insert(std::make_pair(submesh.meshName, submesh));
		tempSubMeshVector[i] = submesh.meshName;
	}

	LoadNodes(scene->mRootNode, tempSubMeshVector);

	// Configure parent's bounding box based off of the submeshes we just added
	{
		std::unordered_map<std::string, Submesh>::iterator it;
		for (it = this->submeshes.begin(); it != this->submeshes.end(); it++)
		{
...
void Mesh::LoadNodes(aiNode* node, std::vector<std::string>& submeshes, const glm::mat4& parentTransform)
{
	...
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
		...
		Submesh& submesh = this->submeshes[submeshes[meshIndex]];
	...
	{
		LoadNodes(node->mChildren[i], submeshes, transform);
	}
}

Submesh Mesh::LoadSubmesh(aiMesh* assimpMesh)
{
	Submesh submesh;
	...
	submesh.vertexArray = CreateRef<VertexArrayObject>();

	uint32_t vertexBufferSize = (uint32_t)(submesh.vertices.size() * sizeof(Vertex));
	float* vertexBuffer = ConvertVerticesToArray(submesh.vertices);

	submesh.vertexBuffer = CreateRef<VertexBuffer>(vertexBuffer, vertexBufferSize);
	submesh.vertexBuffer->SetLayout(this->bufferLayout);

	uint32_t indexBufferSize = (uint32_t)(submesh.faces.size() * sizeof(Face));
	uint32_t* indexBuffer = ConvertIndicesToArray(submesh.faces);

	submesh.vertexArray->Bind();
	submesh.indexBuffer = CreateRef<IndexBuffer>(indexBuffer, indexBufferSize);

	submesh.vertexArray->AddVertexBuffer(submesh.vertexBuffer);
	submesh.vertexArray->SetIndexBuffer(submesh.indexBuffer);
...
void Mesh::AddSubmesh(const std::string& filePath)
{
	std::cout << "Adding submesh " << filePath << "..." << std::endl;;
	...
	LoadNodes(scene->mRootNode, tempSubMeshVector, this->transform); // Load all the submeshes

	// Resize bounding box
	for (const std::string& key : tempSubMeshVector)
	{
		Submesh& submesh = this->submeshes[key];
		AABB& submeshAABB = submesh.boundingBox;
		glm::vec3 min = glm::vec3(submesh.transform * glm::vec4(submeshAABB.min, 1.0f));
		glm::vec3 max = glm::vec3(submesh.transform * glm::vec4(submeshAABB.max, 1.0f));
...
std::vector<Vertex> Mesh::GetVertices() const
{
	...
	for (const std::pair<std::string, Submesh>& entry : this->submeshes)
	{
		const Submesh& submesh = entry.second;
...
std::vector<Face> Mesh::GetFaces() const
{
	...
	{
		const Submesh& submesh = entry.second;
		faces.reserve(submesh.faces.size());
		faces.insert(faces.end(), submesh.faces.begin(), submesh.faces.end());
	}
	return faces;
}